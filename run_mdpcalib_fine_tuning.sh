#!/usr/bin/env bash
# Fine-tuning only pipeline: skips the initial hand-eye calibration stage
# and uses the initial_calibration values from calib_cfg/config/config.yaml.
#
# Before running, edit src/calib_cfg/config/config.yaml and set:
#   skip_initial_calibration: true
#   initial_calibration:
#     tx: <your_value>  ty: <your_value>  tz: <your_value>
#     qw: <your_value>  qx: <your_value>  qy: <your_value>  qz: <your_value>

SESSION_NAME="mdpcalib_fine_tuning"
HELPER_SCRIPT="/tmp/attach_mdpcalib_ft.sh"
...
echo "[attach_mdpcalib_ft] Role: $ROLE"
echo "[attach_mdpcalib_ft] Looking for container starting with: $PREFIX"

# Wait for container to exist
while true; do
    CONTAINER_NAME=$(docker ps --format '{{.Names}}' | grep "^${PREFIX}" | head -n1 || true)
    if [[ -n "${CONTAINER_NAME:-}" ]]; then
        echo "[attach_mdpcalib_ft] Attaching to: $CONTAINER_NAME"
        break
    fi
    echo "[attach_mdpcalib_ft] Waiting for container..."
    sleep 2
done

# Wait for roscore (process-based, robust)
WAIT_ROSCORE='echo "[attach_mdpcalib_ft] Waiting for roscore (process)..."; \
while ! pgrep -f "[r]oscore" >/dev/null 2>&1; do sleep 2; done; \
echo "[attach_mdpcalib_ft] roscore process found.";'
...
echo \"[attach_mdpcalib_ft] Cleaning data/experiments/*\"; \
...
exec bash"
    ...
    sync)
        ROLE_CMD="$WAIT_ROSCORE \
echo \"[attach_mdpcalib_ft] Cleaning /data/cache/*\"; \
rm -rf /data/cache/*; \
...
exec bash"
    ...
    play)
        ROLE_CMD="$WAIT_ROSCORE \
echo \"[attach_mdpcalib_ft] Waiting for ORB vocabulary to be loaded...\"; \
...
# 3: bottom-left     → optimizer (fine-tuning mode, skip_initial_calibration=true)
